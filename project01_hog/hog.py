...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    fin = 0
    if (num_rolls == 0):
        fin = boar_brawl(player_score, opponent_score)
    else:
        fin = roll_dice(num_rolls, dice)
    return fin
    # END PROBLEM 3
...
def sus_update(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    n = player_score
    if(num_rolls == 0):
        n += boar_brawl(player_score, opponent_score)
    else:
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    aver_f = make_averaged(roll_dice, samples_count)
    max = 1
    i = 1
    a = [0]*10
    while(i <= 10):
